...
const ENDPOINTS = {
  ...
  doctorActivate: (doctorId) => `${API_BASE}/admin/doctors/${doctorId}/activate`,
  doctorDeactivate: (doctorId) => `${API_BASE}/admin/doctors/${doctorId}/deactivate`,

  patients: (qs) => `${API_BASE}/admin/patients?${qs}`,
  patientDetails: (patientId) => `${API_BASE}/admin/patients/${patientId}`,
  patientAppointments: (patientId) => `${API_BASE}/admin/patients/${patientId}/appointments`,
  patientActivate: (patientId) => `${API_BASE}/admin/patients/${patientId}/activate`,
  patientDisable: (patientId) => `${API_BASE}/admin/patients/${patientId}/disable`,
...
/* ---------- ROUTER ---------- */
const TITLES = {
  overview: "Overview", doctors: "Doctors", doctorDetail: "Doctor Details",
  patients: "Patients", patientDetail: "Patient Details", clinics: "Clinics",
  subscriptions: "Subscriptions", reports: "Reports", reportDetail: "Report Details",
  settings: "Settings", profile: "Admin Profile",
};
...
function statusPill(status) {
  const label = { pending: "Pending", active: "Active", inactive: "Inactive" }[status] || status;
  return `<span class="pill ${status}"><span class="d"></span>${label}</span>`;
}
function apptStatusPill(status) {
  const label = { waiting: "Waiting", "in-progress": "In Progress", completed: "Completed", cancelled: "Cancelled" }[status] || status;
  return `<span class="pill ${status}">${label}</span>`;
}
function reportStatusPill(status) {
  const label = { pending: "Pending", inreview: "In Review", resolved: "Resolved" }[status] || status;
  return `<span class="pill ${status === "pending" ? "pending" : status}">${label}</span>`;
}
function subStatusPill(status) {
  const label = { active: "Active", expired: "Expired", cancelled: "Cancelled" }[status] || status;
  return `<span class="pill ${status}">${label}</span>`;
}
function fmtDate(d) {
  if (!d) return "—";
...
function renderPagination(el, pagination, onPage) {
  ...
  el.innerHTML = html;
  $$("button[data-p]", el).forEach((b) => b.onclick = () => onPage(+b.dataset.p));
}
...
function confirmModal({ title, body, confirmText = "Confirm", danger = false, onConfirm, extraFieldHtml = "" }) {
  openModal(`
    <h2>${title}</h2>
  ...
  $("#confirmYes").onclick = () => { onConfirm && onConfirm(); };
}
...
async function loadOverview() {
  ...
  try {
    ...
    $("#ovStatGrid").innerHTML = [
      ...
      ["Appointments", d.appointments.total, `${d.appointments.completed} completed`],
      ["Subscriptions", d.subscriptions.total, `${d.subscriptions.active} active`],
      ["Open Reports", d.problemReports.total, `${d.problemReports.pending} pending`],
    ].map(([k, v, sub]) => `
  ...
  } catch { $("#ovPendingDoctors").innerHTML = `<div class="empty-row">Couldn't load.</div>`; }
  ...
  } catch { $("#ovLatestReports").innerHTML = `<div class="empty-row">Couldn't load.</div>`; }
}
...
async function loadDoctors() {
  const qs = new URLSearchParams({
    page: DR_STATE.page, limit: DR_STATE.limit,
  ...
  try {
    ...
    $("#drTableBody").innerHTML = doctors.length ? doctors.map((doc) => `
      <tr>
        <td>
      ...
      </tr>`).join("") : `<tr><td colspan="5" class="empty-row">No doctors found.</td></tr>`;

    renderPagination($("#drPagination"), pagination, (p) => { DR_STATE.page = p; loadDoctors(); });
  } catch (err) {
...
document.addEventListener("click", (e) => {
  ...
  if (el) { openDoctorDetail(el.dataset.openDoctor); }
});
let drSearchTimer;
$("#drSearch").addEventListener("input", (e) => {
  clearTimeout(drSearchTimer);
  drSearchTimer = setTimeout(() => { DR_STATE.search = e.target.value.trim(); DR_STATE.page = 1; loadDoctors(); }, 350);
});
$("#drStatusFilter").addEventListener("change", (e) => { DR_STATE.status = e.target.value; DR_STATE.page = 1; loadDoctors(); });
$("#drSpecFilter").addEventListener("input", (e) => {
  clearTimeout(drSearchTimer);
  drSearchTimer = setTimeout(() => { DR_STATE.spec = e.target.value.trim(); DR_STATE.page = 1; loadDoctors(); }, 350);
});
...
async function openDoctorDetail(doctorId) {
  ...
  try {
    ...
    $("#ddIdSpec").textContent = `${doctor.doctorId} · ${doctor.specialization}`;

    $("#ddInfoGrid").innerHTML = [
      ["Email", doctor.email], ["Phone", doctor.phone],
      ["Clinic", doctor.clinicName], ["Clinic Address", doctor.clinicAddress],
      ["Experience", `${doctor.experience} years`], ["License Number", doctor.licenseNumber],
      ["Consultation Fee", `PKR ${Number(doctor.consultationFee || 0).toLocaleString()}`],
    ...
    $("#ddSubGrid").innerHTML = subscription
      ? [
          ["Plan", `<span class="pill ${subscription.plan}">${subscription.plan === "paid" ? "Paid" : "Free"}</span>`],
          ["Status", subStatusPill(subscription.status)],
          ["Start Date", fmtDate(subscription.startDate)],
          [subscription.status === "expired" ? "Expired" : "Expires", subscription.plan === "paid" ? fmtDate(subscription.endDate) : "—"],
        ].map(([k, v]) => `<div><div class="fk">${k}</div><div class="fv">${v}</div></div>`).join("")
      : `<div class="empty-row">No subscription on record.</div>`;

    // Actions — only Verify/Deactivate/Activate, per spec (never "Suspend")
    let actions = "";
    if (doctor.status === "pending") actions = `<button class="btn btn-primary btn-sm" id="ddApprove">Verify / Accept</button>`;
    else if (doctor.status === "active") actions = `<button class="btn btn-danger btn-sm" id="ddDeactivate">Deactivate</button>`;
    else if (doctor.status === "inactive") actions = `<button class="btn btn-primary btn-sm" id="ddActivate">Activate</button>`;
    $("#ddActions").innerHTML = actions;

    if ($("#ddApprove")) $("#ddApprove").onclick = () => doctorAction(ENDPOINTS.doctorApprove(doctorId), "Doctor approved");
    if ($("#ddDeactivate")) $("#ddDeactivate").onclick = () =>
      confirmModal({
        title: "Deactivate this doctor?", body: "They will no longer be able to use ClinicFlow until reactivated.",
        confirmText: "Deactivate", danger: true,
        onConfirm: () => { closeModal(); doctorAction(ENDPOINTS.doctorDeactivate(doctorId), "Doctor deactivated"); },
      });
    if ($("#ddActivate")) $("#ddActivate").onclick = () => doctorAction(ENDPOINTS.doctorActivate(doctorId), "Doctor activated");
...
async function loadDoctorAppointments(doctorId, date) {
  try {
    ...
    $("#ddApptBody").innerHTML = appointments.length
      ? appointments.map((a) => `
        <tr><td>${a.patientName}</td><td>${fmtDate(a.appointmentDate)}</td><td>${apptStatusPill(a.status)}</td></tr>`).join("")
      : `<tr><td colspan="3" class="empty-row">No appointments on this date.</td></tr>`;
  } catch (err) {
    toast("Couldn't load appointments", err.message, true);
  }
}
$("#ddApptDate").addEventListener("change", (e) => loadDoctorAppointments(currentDoctorId, e.target.value));

/* ================= PATIENTS LIST ================= */
async function loadPatients() {
  const qs = new URLSearchParams({
    page: PT_STATE.page, limit: PT_STATE.limit,
  ...
  try {
    ...
    $("#ptTableBody").innerHTML = patients.length ? patients.map((p) => `
      <tr>
      ...
      </tr>`).join("") : `<tr><td colspan="5" class="empty-row">No patients found.</td></tr>`;
    renderPagination($("#ptPagination"), pagination, (pg) => { PT_STATE.page = pg; loadPatients(); });
  } catch (err) {
...
$("#ptSearch").addEventListener("input", (e) => {
  ...
  ptSearchTimer = setTimeout(() => { PT_STATE.search = e.target.value.trim(); PT_STATE.page = 1; loadPatients(); }, 350);
});
$("#ptStatusFilter").addEventListener("change", (e) => { PT_STATE.status = e.target.value; PT_STATE.page = 1; loadPatients(); });
...
async function openPatientDetail(patientId) {
  ...
  try {
    ...
    if ($("#pdDisable")) $("#pdDisable").onclick = () =>
      confirmModal({
        title: "Disable this patient?", body: "They will not be able to book new appointments until reactivated.",
        confirmText: "Disable", danger: true,
        onConfirm: () => { closeModal(); patientAction(ENDPOINTS.patientDisable(patientId), "Patient disabled"); },
      });
    if ($("#pdActivate")) $("#pdActivate").onclick = () => patientAction(ENDPOINTS.patientActivate(patientId), "Patient activated");
    ...
    $("#pdApptBody").innerHTML = list.length
      ? list.map((a) => `<tr><td>${a.doctor?.name || "—"}</td><td>${a.clinic || "—"}</td><td>${fmtDate(a.appointmentDate)}</td><td>${apptStatusPill(a.status)}</td></tr>`).join("")
...
async function loadSubscriptions() {
  const qs = new URLSearchParams({
    page: SUB_STATE.page, limit: SUB_STATE.limit,
  ...
  try {
    ...
    if (SUB_STATE.search) {
      ...
      subscriptions = subscriptions.filter((s) => (s.doctor?.user?.fullname || "").toLowerCase().includes(q));
    }

    $("#subTableBody").innerHTML = subscriptions.length ? subscriptions.map((s) => `
      <tr>
      ...
      </tr>`).join("") : `<tr><td colspan="5" class="empty-row">No subscriptions found.</td></tr>`;

    renderPagination($("#subPagination"), pagination, (p) => { SUB_STATE.page = p; loadSubscriptions(); });
  } catch (err) {
...
$("#subSearch").addEventListener("input", (e) => {
  ...
  subSearchTimer = setTimeout(() => { SUB_STATE.search = e.target.value.trim(); loadSubscriptions(); }, 350);
});
$("#subPlanFilter").addEventListener("change", (e) => { SUB_STATE.plan = e.target.value; SUB_STATE.page = 1; loadSubscriptions(); });
$("#subStatusFilter").addEventListener("change", (e) => { SUB_STATE.status = e.target.value; SUB_STATE.page = 1; loadSubscriptions(); });
...
async function openSubscriptionModal(subscriptionId) {
  try {
    ...
    $("#subExtendBtn").onclick = async () => {
      try {
        await apiPatch(ENDPOINTS.subscriptionExtend(subscriptionId), { days: 30 });
        toast("Subscription extended");
        closeModal(); loadSubscriptions();
      } catch (err) { toast("Couldn't extend", err.message, true); }
    };
    $("#subTogglePlanBtn").onclick = async () => {
      try {
        await apiPatch(ENDPOINTS.subscriptionPlan(subscriptionId), { plan: s.plan === "paid" ? "free" : "paid" });
        toast("Plan updated");
        closeModal(); loadSubscriptions();
      } catch (err) { toast("Couldn't update plan", err.message, true); }
    };
  } catch (err) {
...
async function loadReports() {
  const qs = new URLSearchParams({
    page: REP_STATE.page, limit: REP_STATE.limit,
  ...
  try {
    ...
    $("#repStatGrid").innerHTML = [
      ["Pending", counts.pending], ["In Review", counts.inreview], ["Resolved", counts.resolved],
    ].map(([k, v]) => `<div class="stat-card"><div class="k">${k}</div><div class="v">${v}</div></div>`).join("");

    $("#repTableBody").innerHTML = reports.length ? reports.map((r) => `
      <tr>
      ...
      </tr>`).join("") : `<tr><td colspan="5" class="empty-row">No reports found.</td></tr>`;

    renderPagination($("#repPagination"), pagination, (p) => { REP_STATE.page = p; loadReports(); });
  } catch (err) {
...
$("#repStatusFilter").addEventListener("change", (e) => { REP_STATE.status = e.target.value; REP_STATE.page = 1; loadReports(); });
...
async function openReportDetail(reportId) {
  ...
  try {
    ...
    let actionHtml = "";
    if (r.status === "pending") actionHtml = `<button class="btn btn-primary" id="rdReviewBtn">Move to Review</button>`;
    else if (r.status === "inreview") actionHtml = `<button class="btn btn-primary" id="rdResolveBtn">Resolve</button>`;
    ...
    if ($("#rdReviewBtn")) $("#rdReviewBtn").onclick = async () => {
      try { await apiPatch(ENDPOINTS.reportReview(reportId)); toast("Moved to review"); openReportDetail(reportId); }
      catch (err) { toast("Action failed", err.message, true); }
    };
    if ($("#rdResolveBtn")) $("#rdResolveBtn").onclick = () => {
      confirmModal({
        title: "Resolve this report", body: "Describe how this was resolved.",
        confirmText: "Resolve",
        ...
        onConfirm: async () => {
          ...
          try {
            ...
            closeModal(); toast("Report resolved"); openReportDetail(reportId);
          } catch (err) { toast("Action failed", err.message, true); }
        },
      });
    };
...
async function loadProfile() {
  try {
    ...
    const p = res.data;
    ADMIN.fullname = p.fullname; ADMIN.email = p.email; ADMIN.phone = p.phone; ADMIN.status = p.status;
    syncTopbarIdentity();

    $("#profName").textContent = p.fullname;
    $("#profStatus").textContent = p.status === "active" ? "Active administrator account" : "Inactive account";
    $("#profGrid").innerHTML = [
      ["Admin ID", p.adminId], ["Email", p.email],
    ].map(([k, v]) => `<div><div class="fk">${k}</div><div class="fv">${v}</div></div>`).join("");
...
$("#profForm").addEventListener("submit", async (e) => {
  ...
  try {
    await apiPatch(ENDPOINTS.profile(), { fullname: $("#profFullname").value.trim(), phone: $("#profPhone").value.trim() });
    toast("Profile updated");
    loadProfile();
  } catch (err) { toast("Couldn't update profile", err.message, true); }
...
$("#pwForm").addEventListener("submit", async (e) => {
  ...
  try {
    await apiPatch(ENDPOINTS.password(), { currentPassword: $("#pwCurrent").value, newPassword: $("#pwNew").value });
    toast("Password updated");
    e.target.reset();
  } catch (err) { toast("Couldn't update password", err.message, true); }
...
$("#logoutBtn").addEventListener("click", async () => {
  try { await apiCall(ENDPOINTS.logout(), { method: "POST" }); }
  catch (err) { console.warn("Logout request failed, redirecting anyway:", err.message); }
...
async function init() {
  try {
    ...
    ADMIN.fullname = res.data.fullname; ADMIN.email = res.data.email;
    syncTopbarIdentity();
  } catch (err) {
    if (err.status === 401 || err.status === 403) { window.location.href = LOGIN_PATH; return; }
